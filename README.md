...
- ✅ **상세 정류장 검색**: 개별 정류장 정보, 관계 분석, 비교 기능 제공
- ✅ **네트워크 분석**: 정류장 간 관계, 중심성 지표, Voronoi 다이어그램
...
```bash
# 필수 패키지 설치
pip install pandas numpy matplotlib seaborn pulp folium plotly streamlit streamlit-folium scipy scikit-learn networkx
...
# 3단계: 고급 시각화 생성 (네트워크, Voronoi 등)
python 고급_시각화_도구.py

# 4단계: 기본 대시보드 실행
streamlit run 전문가_대시보드.py

# 5단계: 고도화 대시보드 실행 (상세 정류장 분석)
streamlit run 고도화_대시보드.py
```
...
#### 고급 시각화
- `고급시각화_네트워크.png` - 정류장 네트워크 분석 및 중심성 지표
- `고급시각화_Voronoi.png` - Voronoi 다이어그램 (서비스 권역)
- `고급시각화_거리행렬.png` - 주요 정류장 간 거리 매트릭스
- `고급시각화_3D표면.png` - 3D 수요 밀도 표면 플롯
- `고급시각화_클러스터.png` - K-means 클러스터링 분석
...
├── 고품질_시각화.py               # 기본 시각화 생성
├── 고급_시각화_도구.py           # 고급 시각화 (네트워크, Voronoi 등)
├── 전문가_대시보드.py             # Streamlit 기본 대시보드
├── 고도화_대시보드.py             # Streamlit 고도화 대시보드 (상세 분석)
├── requirements.txt              # 필수 패키지
├── 분석결과_*.csv                # 분석 결과 데이터
├── 보고서_*.png/html             # 기본 시각화 결과
├── 고급시각화_*.png              # 고급 시각화 결과
└── README.md                     # 본 문서
```
...
- **네트워크 분석**: NetworkX (중심성 지표, 그래프 이론)
- **공간 분석**: SciPy Spatial (Voronoi, 거리 계산)
- **대시보드**: Streamlit, Streamlit-Folium
- **기계학습**: Scikit-learn (DBSCAN, K-means 클러스터링)
...
**최종 업데이트**: 2025-12-11
**버전**: 2.1.0 (정류장 상세 분석 및 네트워크 시각화 추가)
